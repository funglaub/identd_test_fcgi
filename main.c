...
int query_identd(char *buffer, char *remote_addr, char *remote_port) {
  ...
  // Request should hold SOURCE_PORT,80
  request = malloc(strlen(remote_port)+strlen(www_port)+2); // ',' and '\0'
  snprintf(request, sizeof request, "%s", remote_port);
  snprintf(request, sizeof request, "%s", www_port);
